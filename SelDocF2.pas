...
uses
  ...
  VCIF1Lib_TLB, TB2Item, RzButton, RzRadChk;

{****************************************************************
Definicion de Objetos que se regresaran
****************************************************************}
type
  TfrmSelDocF2 = class(TForm)
    pnlMain: TPanel;
...
procedure TfrmSelDocF2.FormCreate(Sender: TObject);
begin
//    Setup;
//    Open;
...
procedure TfrmSelDocF2.FormActivate(Sender: TObject);
begin
    Setup;
    Open;
    F1Doc.SetFocus;
...
procedure TfrmSelDocF2.SetUp;
begin
    frmSelDocF2.Caption := 'Documentos de ' + CodCteProv + ' ' + RazSocial;
    if (Tipo_Docto = '23') or (Tipo_Docto = '232') or (Tipo_Docto = '233') then begin
        frmSelDocF2.Caption := 'Pedidos de ' + CodCteProv + ' ' + RazSocial;
    end;
    if (Tipo_Docto = '28') or (Tipo_Docto = '282') or (Tipo_Docto = '283') then begin
        frmSelDocF2.Caption := 'Remisiones de ' + CodCteProv + ' ' + RazSocial;
    end;
    if (Tipo_Docto = '30') or (Tipo_Docto = '302') or (Tipo_Docto = '303') then begin
        frmSelDocF2.Caption := 'Cotizaciones de ' + CodCteProv + ' ' + RazSocial;
    end;
    if (Tipo_Docto = '31') then begin
        frmSelDocF2.Caption := 'Compras:  ' + CodCteProv + ' ' + RazSocial;
    end;
    if (Tipo_Docto = '63') then begin
        frmSelDocF2.Caption := 'Ordene de Compra:  ' + CodCteProv + ' ' + RazSocial;
    end;
    if (Tipo_Docto = '81') then begin
        frmSelDocF2.Caption := 'Entradas de Almacen:  ' + CodCteProv + ' ' + RazSocial;
    end;
...
begin
   ...
   icuDoc.SQL.Clear;
   icuDoc.SQl.add ('Select * from mba10008_s15(?p_NumTipoDoc,?p_FechaIni,?p_FechaFin);');
   icuDoc.Prepare;
   icuDoc.ParamByName('P_NUMTIPODOC').AsString := Tipo_Docto;
   icuDoc.ParamByName('P_FECHAINI').AsDateTime := FechaIni;
   icuDoc.ParamByName('P_FECHAFIN').AsDateTime := FechaFin;
...
procedure TfrmSelDocF2.OpenDetalle;
var iRen:integer;
...
procedure TfrmSelDocF2.UnPrepare;
begin
    icuDoc.Close;
    icuDoc.unprepare;
    icuDet.Close;
...
procedure TfrmSelDocF2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
    Action := caHide;
end;

procedure TfrmSelDocF2.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
case Key of
    VK_RETURN: Aceptar;
...
procedure TfrmSelDocF2.Aceptar;
var i:Integer;
begin
    ModalResult := mrOk;
end;
procedure TfrmSelDocF2.Cancelar;
begin
    ModalResult := mrCancel;
end;

procedure TfrmSelDocF2.tlbAceptarClick(Sender: TObject);
begin
    Aceptar;
end;

procedure TfrmSelDocF2.tbtCAncelarClick(Sender: TObject);
begin
    Cancelar;
end;

procedure TfrmSelDocF2.tlbRefrescarDocClick(Sender: TObject);
begin
    Open;
end;


procedure TfrmSelDocF2.edtKeyChange(Sender: TObject);
var Campox:String;
begin
end;

procedure TfrmSelDocF2.F1DocSelChange(Sender: TObject);
begin
    if miRen <= 0 then
        miRen := F1Doc.Row;
...
procedure TfrmSelDocF2.Seleccion;
var i:Integer;
    iIni,iFin:Integer;
begin
...
procedure TfrmSelDocF2.Invertir;
var i:Integer;
begin
    for i:= miRenIni to DetCount do begin
        if F1Det.TextRC[i,1] = mtOk then
...
procedure TfrmSelDocF2.tbtTodosClick(Sender: TObject);
begin
    Todo;
end;

procedure TfrmSelDocF2.tbtNingunoClick(Sender: TObject);
begin
    Nada;
end;

procedure TfrmSelDocF2.tbtSelClick(Sender: TObject);
begin
    Seleccion;
end;

procedure TfrmSelDocF2.tbtInvertirClick(Sender: TObject);
begin
    Invertir;
end;

procedure TfrmSelDocF2.F1DetKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfrmSelDocF2.F1DetStartEdit(Sender: TObject;
  var EditString: WideString; var Cancel: Smallint);
begin
    if F1Det.Col <> 3 then Cancel := 1;
end;

procedure TfrmSelDocF2.tbtRefrescarDetClick(Sender: TObject);
begin
    OpenDetalle;
end;

procedure TfrmSelDocF2.F1DetDblClick(Sender: TObject; nRow, nCol: Integer);
begin
    Seleccion;
end;

procedure TfrmSelDocF2.chkPendientesClick(Sender: TObject);
begin
    OpenDetalle;
end;
